...
func buildWorkerBindings(kvNs *kv.Namespace, bindings []SecretBinding) []map[string]string {
	var envVars []map[string]string

	if kvNs != nil {
		envVars = append(envVars, map[string]string{
			"name":         "kv",
			"namespace_id": kvNs.ID,
			"type":         "kv_namespace",
		})
	}
...
func createWorker(ctx context.Context, name string, jsContent []byte, kvNs *kv.Namespace, bindings []SecretBinding) (*workers.ScriptUpdateResponse, error) {
	envVars := buildWorkerBindings(kvNs, bindings)
...
func updateWorker(ctx context.Context, name string) error {
	...
	_, err = cfClient.Workers.Scripts.Content.Update(
		ctx,
		name,
	...
	if err != nil {
		return fmt.Errorf("error updating worker script: %w", err)
	}
...
func deployWorker(
	...
	for {
		...
		_, err := createWorker(ctx, name, jsContent, kvNamespace, bindings)
		if err != nil {
